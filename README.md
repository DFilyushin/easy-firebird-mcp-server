...
| `get_schema_digest`    | Вся схема (таблицы, колонки, PK, индексы, VIEW, сигнатуры процедур) одним вызовом + `fingerprint` |
| `get_schema_fingerprint`| Короткий хэш схемы одним дешёвым запросом — для проверки актуальности кэша |
...
## Кэширование схемы между сессиями

Чтобы агент не переоткрывал структуру БД в начале каждой сессии (десятки вызовов
`describe_table` / `get_procedure_source` — это долго и дорого по токенам), используйте
пару `get_schema_digest` + `get_schema_fingerprint`:

1. **Один раз** вызовите `get_schema_digest` и сохраните результат в репозиторий —
   например в `db_schema.json`, — а в `CLAUDE.md` добавьте ссылку на него.
2. В `CLAUDE.md` пропишите правило для агента:

   > Схема БД описана в `db_schema.json` (поле `fingerprint` — её версия).
   > В начале работы вызови `get_schema_fingerprint`. Если он совпадает с `fingerprint`
   > в файле — схема актуальна, читай её из файла и **не** обращайся к БД за метаданными.
   > При расхождении — перечитай `get_schema_digest` и обнови файл.

Так схема попадает в контекст агента из закоммиченного файла, а в БД он ходит только
когда `get_schema_fingerprint` показал, что схема действительно изменилась
(`fingerprint` меняется при `ALTER`/`CREATE`/`DROP` таблиц и процедур).
